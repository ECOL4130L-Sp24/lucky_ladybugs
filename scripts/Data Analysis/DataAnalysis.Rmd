...
```
A linear regression was performed to predict the relationship between surface water nitrogen and algal biomass. The results of the linear regression indicated that surface water nitrogen explained -11.94% of the variation in algal biomass (F(1,6)=0.2535, p=0.6326).
```{r}
linear_model2 <- lm(mean_weight ~ mean_adjAshFreeDryMass, data = merged_data_all)
summary(linear_model2)
```
A linear regression was performed to predict the relationship between fish abundance and algal biomass. The results of the linear regression indicated that fish abundance explained 24.96% of the variation in algal biomass (F(1,9)=4.327, p=0.06725). 
```{r}
